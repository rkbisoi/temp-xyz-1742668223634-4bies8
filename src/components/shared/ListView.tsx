...
import { InvitationItem, Item, ProjectItem } from '../../types';
...
import { ActivateUsers, delinkUserFromProjects } from '../../services/userService';
import { deleteData, delinkDataFromProjects } from '../../services/dataService';
...
export default function ListView({ items, config, onItemView, onAddItem, projectId = -1, defaultView = 'grid' }: ListViewProps) {
  ...
  const handleLinkAction = async () => {
    ...
    if (getItemType(items[0]) === 'KnowledgeItem') {
        ...
        try {
          ...
          switch (typ) {
            ...
            case "KnowledgeItem":
              ...
              break;
            case "DataItem":
              success = await deleteData(selectedItemList);
              break;
            default:
  ...
  var filteredBulkActions = config?.bulkActions?.filter(action =>
    action.action !== 'delink' || (projectId > 0)
  );

  filteredBulkActions = filteredBulkActions?.filter(action =>
    action.action !== 'link' || (projectId === -1)
  );

  return (
    <div className="px-4 py-2 px-4-8 mt-2">
                    ...
                    <td className={`px-4 py-4 text-right ${config.showSchedule ? 'pr-4' : 'pr-10'}`} onClick={(e) => e.stopPropagation()}>
                      <div className="relative">
                        ...
                        {dropdownOpen === item.id && (
                          <div className="absolute top-4 right-0 mt-2 w-28 bg-white border rounded shadow-md z-50">
                            <ul>
                              <li
                                onClick={() => {
                                  onItemView(item.id);
